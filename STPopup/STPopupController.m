...
- (void)updateNavigationBarAniamted:(BOOL)animated
{
	...
	CGFloat statusBarHeight = [UIApplication sharedApplication].statusBarFrame.size.height / 2;
	...
	if (animated) {
		...
		if (self.style == STPopupStyleFormSheetOuterNavigation) {
			[_containerViewController.view addSubview:toTitleView];
			toTitleView.center = CGPointMake(_containerViewController.view.center.x, (self.positionY/2) + statusBarHeight);
		} else {
		...
		if (topViewController.navigationItem.titleView) {
			...
			if (self.style == STPopupStyleFormSheetOuterNavigation) {
				[_containerViewController.view addSubview:_defaultTitleLabel];
				_defaultTitleLabel.center = CGPointMake(_containerViewController.view.center.x, (self.positionY/2)+statusBarHeight);
			} else {
...
- (void)layoutContainerView
{
	...
	CGFloat containerViewY = (self.positionY > 0) ? self.positionY : (_containerViewController.view.bounds.size.height - containerViewHeight) / 2;
...
-(void)dismissTitleAnimated
{
	__weak UILabel *weekFromLabel = _defaultTitleLabel;
	[UIView animateWithDuration:0.25 animations:^{
		weekFromLabel.alpha = 0;
	} completion:^(BOOL finished) {
		[weekFromLabel removeFromSuperview];
	}];
}
